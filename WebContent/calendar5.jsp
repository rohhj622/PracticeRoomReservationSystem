...
%>
	...
	<tr>
	    <td>
		    <a href="./teacher01.jsp?y=<%=prevYear%>&m=<%=prevMonth%>">◁</a> 
		    <%=year%>년 <%=month+1 %>월 
		    <a href="./teacher01.jsp?y=<%=nextYear%>&m=<%=nextMonth%>">▷</a>
	    </td>
	</tr>
	<tr>
	    <td>
	    		...
	    		if(nowY == cal.get(Calendar.YEAR) && nowM  == cal.get(Calendar.MONTH) && nowD <= cal.get(Calendar.DATE)){
	    			%>
	    				<td class="tb2-td" style="cursor:pointer;" > 
					    	<a href="teacher01.jsp?y=<%=year %>&m=<%=month+1%>&day=<%= cal.get(Calendar.DATE) %>" >
